  ...
  <div id="ask">
        ...
        <div class="content" @click="cancel">
            ...
            <div :class="{item:value.direction==0,item1:!(value.direction==0)}" v-for="(value,index) in allResult" :key="index">
                ...
                <p :class="{span_right:value.direction==0,span_left:!(value.direction==0)}" v-if="value.message!=''">{{value.message}}</p>
        ...
        <div :class="{footer:true,hei:this.add_show}">
            <div :class="{chat:this.add_show}">
                ...
                <input type="text" :class="{text:true,huanyuan:!this.add_show}" v-model="text" @keyup.enter="send" @focus="add_show=false">
                <span :class="{'zk-icon-biaoqing':true, icon:true, biaoqing:true,huanyuan:!this.add_show}"></span>
                <span :class="{'zk-icon-tianjia':true, icon:true, tianjia:true,huanyuan:!this.add_show}" v-if="!isSend" @click="add"></span>
                <input type="button" :class="{btn:true,huanyuan:!this.add_show}" value="发送" v-if="isSend" @click="send">
            </div>
            <div class="add_box" v-show="add_show">
                <Upload @changeUrls="getUrl" img-max-num="9" :file-type=1 sign="headimg" title="相册"><span class="text"></span></Upload>
                <!-- <p></p> -->
            </div>
        </div>
  </div>
