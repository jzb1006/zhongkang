<template>
    <div id="diarySelItem" :style="{height:clientHeight}" v-show="status">
        ...
        <p class="items">
            <span class="detail_selected" v-for="ch in choose" @click="sel_item3(ch.id)">{{ch.name}}</span>
            <span class="detail_selected" v-if="choose.length <= 0">请至少选择一个项目</span>
        </p>
        <div class="show_sel clearfix ">
            <div class="col_4 show_items">
                <ul class="sel_item">
                    <li v-for="item in itemList" v-if="item.parent_id == 0" @click="sel_item1(item.id)" :class = "{item1_selected:item.id == item1}">{{item.name}}</li>
                </ul>
            </div>
