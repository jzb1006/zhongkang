<template>
    <div id="upload">
        <div class="classics" v-if="uploadSel == 'classics'">
            <span class="btn btn-sm btn-outline-info position-relative">
                <form id="form1" enctype="multipart/form-data">
                    <input type="file" name="sf_upfile" @change="changeFile($event)" class="position-absolute upfile btn btn-sm btn-outline-info">
                    {{uploadText[fileType-1]}}
                </form>
            </span>
        </div>
...
export default {
    props: ["uploadSel", "img-max-num", "video-max-num", "file-type"],
    data() {
        return {
            fileUrls: [],
            initFiles: [],
